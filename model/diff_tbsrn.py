'''
Transformer-Based Super Resolution Networks
'''
import math
import torch
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        # print('Attention shape:', self.to_out(out).shape)
        return self.to_out(out) # output
...
class FeedForward(nn.Module):
    ...
    def forward(self, x, t):
        ...
        x = x * (scale + 1) + shift
        # print('FF shape', self.proj_out(x).shape)
...
class TBSRN(nn.Module):
    def __init__(self, 
                 ...
                 height=32,
                 STN=True):
                #  width=128, height=32, STN=True, srb_nums=5, mask=False, hidden_units=32, input_channel=3):
        ...
        print('init_img_transform:',init_img_transform)
        print('fianl_img_transform:',final_img_itransform)
    ...
    def forward(self, x, time):
        ...
        x = self.unpatchify(x)
        return self.final_img_itransform(x) # TBSRN output
...
if __name__ == '__main__':
    print("success")
    config = yaml.load(open('/data1/hom1/ict17/FudanOCR/STT_Diffusion/config/super_resolution.yaml', 'r'), Loader=yaml.Loader)
    cfg = EasyDict(config)
    cfg.TRAIN
    # model = TBSRN(scale_factor=cfg.TRAIN.down_sample_scale, width=cfg.TRAIN.width, height=cfg.TRAIN.height,
    #                           STN=False, mask=False, srb_nums=5, hidden_units=32)
